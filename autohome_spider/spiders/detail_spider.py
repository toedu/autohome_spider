...
class Detail(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if not matcher:
            # logging.msg('modelId[%s], no data...' % current)
            return
        data = matcher.group(1)
        model_ids = data.split(',')
        if str(current) not in model_ids:
            logging.msg('modelId[%s], no current data...' % current)
            return
    ...
    def readIds(self):
        ...
        print names
        if not names:
            logging.msg('[spec]no model data file in data dir.', log.ERROR)
            return
